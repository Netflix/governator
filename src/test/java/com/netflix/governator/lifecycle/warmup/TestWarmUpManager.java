...
public class TestWarmUpManager {
  ...
  @Test
  public void testDag1Module() throws Exception {
  ...
  private void assertOrdering(Recorder recorder, String base, String dependency) {
    ...
    Assert
        .assertTrue(baseIndex > dependencyIndex, "baseIndex: " + baseIndex + " - dependencyIndex: " + dependencyIndex);
  }

  private void assertNotConcurrent(Recorder recorder, String task1, String task2) {
    for (Set<String> s : recorder.getConcurrents()) {
      Assert.assertTrue(!s.contains(task1) || !s.contains(task2),
          String.format("Incorrect concurrency for %s and %s: %s", task1, task2, s));
    }
  }
}
